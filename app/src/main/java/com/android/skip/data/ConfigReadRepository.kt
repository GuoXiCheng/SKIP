...
class ConfigReadRepository @Inject constructor() {
    ...
    private var configList: MutableList<ConfigReadSchema> = mutableListOf()

    fun readConfig(configReadSchemaList: List<ConfigReadSchema>) {
        val gson = Gson()
        val jsonStr = gson.toJson(configReadSchemaList)

        val type = object : TypeToken<List<ConfigReadSchema>>() {}.type
        configList = gson.fromJson<List<ConfigReadSchema>>(jsonStr, type).toMutableList()

        LogUtils.d(jsonStr.hashCode())
        _configHashCode.postValue(jsonStr.hashCode())
    }

    fun loadConfig(): Map<String, ConfigLoadSchema> {
        val screenWidth = ScreenUtils.getScreenWidth()
        val screenHeight = ScreenUtils.getScreenHeight()

        return configList.associate { config ->
